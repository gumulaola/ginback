...
_TASK = 1  # Pre-setting: 1 for task1 and 2 for task2
...
class CachingServer(TCPServer):
    ...
    def generateBuffer(self, body: Type[io.BufferedIOBase], store=False, key=None):
        ''' A generator to yield the body buffer by buffer. Used in task2.
        Params:
            body: body to be generated or may be stored (depends on `store`)
            store: whether to be stored to cacheTable
            key: key of the item. Used when `store` == True.
        Yield:
            a bytearray buffer
        '''
        ...
        while l > 0:
            l = body.readinto(buf)
            if store:
                self.cacheTable.appendBody(key, buf[:l])
            yield buf[:l]
    ...
    def touchItem(self, path: str):
        ...
        def task1():
            ''' Store the target locally, then return it '''
            if path not in self.cacheTable or \
               self.cacheTable.expired(path):    # cache miss or expires 
                response: Optional[HTTPResponse] = self.requestMainServer(path)
                if response is None:
                    return None

                headers = self._filterHeaders(response.getheaders())
                self.cacheTable.setHeaders(path, headers)
                self.cacheTable.appendBody(path, response.read())
                # self.cacheTable[path] = headers, response.read()  # cache it
            else:
                self.log_info(f"Request for '{path}' hit cache")    
            return self.cacheTable.getBody(path)

        def task2():
            ''' For large data, when receiving, store in cache and send to client
            at the same time.
            Return a generator for HttpHandler to send out the data.
            '''
            if path not in self.cacheTable or \
               self.cacheTable.expired(path):    # cache miss or expires
                response: Optional[HTTPResponse] = self.requestMainServer(path)
                ...
                headers = self._filterHeaders(response.getheaders())
                self.cacheTable.setHeaders(path, headers)  # store in cache for later use
                return self.generateBuffer(response, store=True, key=path)
            else:
                self.log_info(f"Request for '{path}' hit cache")
                body = io.BytesIO(self.cacheTable.getBody(path))  # subclass of BufferedIOBase
                return self.generateBuffer(body)
        tasks = [task1, task2]
        return tasks[_TASK-1]()
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    There is two io.BufferedIOBase readable and writable objects, self.rfile
    ...
    def sendHeaders(self):
        ...
        # simply use the headers in the cache to send HTTP response
        headers = self.server.cacheTable.getHeaders(self.path)
    ...
    def do_GET(self):
        ...
        def task1():
            body = self.server.touchItem(self.path)
            if body:
                self.sendHeaders()
                self.sendBody(body)
            else:
        ...
        def task2():
            try:
                bodyGenerator = self.server.touchItem(self.path)
            except FileNotFoundError:
                self.send_error(HTTPStatus.NOT_FOUND, "'File not found'")
                return
            self.sendHeaders()
            for buf in bodyGenerator:
                self.sendBody(buf)
